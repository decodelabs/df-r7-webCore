...
// Date
    public function addDateField($list) {
        $list->addField('date', function($error) {
            return $this->import->component('ErrorLink', '~admin/system/critical-errors/', $error);
        });
    }
...
// Actions
    public function addActionsField($list) {
        $list->addField('actions', function($error) {
            return [
                // Delete
                $this->import->component('ErrorLink', '~admin/system/critical-errors/', $error, $this->_('Delete'))
                    ->setAction('delete')
            ];
        });
    }
